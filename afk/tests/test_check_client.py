...
def test_active_recently(plugin, joe):
    ...
    now = time()
    joe.connects(1)
    plugin.on_client_activity(Event("", None, client=joe), when=(now - 20))  # some activity 20s ago
    # WHEN
...
def test_not_active_recently(plugin, joe):
    ...
    now = time()
    joe.connects(1)
    plugin.on_client_activity(Event("", None, client=joe), when=(now - 50))  # some activity 50s ago
    # WHEN
    plugin.check_client(joe)
...
def test_not_active_recently_but_superadmin(plugin, superadmin):
    ...
    plugin.on_client_activity(Event("", None, client=superadmin), when=(now - 50))  # some activity 50s ago
    # WHEN
...
def test_not_active_recently_but_bot(plugin, bot):
    ...
    bot.connects(1)
    bot.bot = True
    plugin.on_client_activity(Event("", None, client=bot), when=(now - 50))  # some activity 50s ago
    # WHEN
